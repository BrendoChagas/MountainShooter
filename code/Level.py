...
class Level:
    def __init__(self, window: Surface, name: str, game_mode: str, player_score: list[int]):
        # Define o tempo limite do nível
        self.timeout = TIMEOUT_LEVEL
        self.window = window
        self.name = name
        self.game_mode = game_mode
        self.entity_list: list[Entity] = []
        # Carrega o background do nível
        ...
        if game_mode in [MENU_OPTION[1], MENU_OPTION[2]]:
            ...
            self.entity_list.append(player)
        # Configura os eventos de spawn de inimigos e timeout
        pygame.time.set_timer(EVENT_ENEMY, SPAWN_TIME)
        pygame.time.set_timer(EVENT_TIMEOUT, TIMEOUT_STEP)  # 100ms

        # Ajusta a duração do Level 3 para o dobro
        if self.name == 'Level3':
            self.timeout *= 2  # Duração em dobro para o Level 3

    def run(self, player_score: list[int]):
        # Carrega e reproduz a música do nível
        ...
        while True:
            ...
            # Desenha e atualiza as entidades
            ...
            for event in pygame.event.get():
                ...
                if event.type == EVENT_ENEMY:
                    if self.name == 'Level3':
                        # No Level 3, gera apenas Enemy3
                        self.entity_list.append(EntityFactory.get_entity('Enemy3'))
                    else:
                        # Nos outros níveis, gera Enemy1 ou Enemy2
                ...
                if event.type == EVENT_TIMEOUT:
                    ...
                    if self.timeout == 0:
                        # Salva o score dos jogadores ao término do nível
                        for ent in self.entity_list:
                ...
                if not found_player:
                    # Se não há jogadores, fim de jogo
                    return False
            ...
            pygame.display.flip()
            # Verifica colisões e atualiza a saúde das entidades
